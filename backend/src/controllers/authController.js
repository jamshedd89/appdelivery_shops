...
const sendCodeValidation = [body('phone').matches(/^\d{9}$/).withMessage('Номер телефона должен содержать 9 цифр')];
...
const verifyCodeValidation = [body('phone').matches(/^\d{9}$/), body('code').notEmpty()];
...
const registerSellerValidation = [
  body('phone').matches(/^\d{9}$/).withMessage('Номер: 9 цифр'), body('password').isLength({ min: 6 }), body('first_name').notEmpty(),
...
const loginValidation = [body('phone').matches(/^\d{9}$/).withMessage('Номер: 9 цифр'), body('password').notEmpty()];
...
async function updatePushToken(req, res, next) {
  try {
    ...
    await User.update({ push_token: req.body.token }, { where: { id: req.user.id } });
    res.json({ success: true });
  } catch (e) { next(e); }
}

module.exports = { sendCode, sendCodeValidation, verifyCode, verifyCodeValidation, registerSeller, registerSellerValidation, registerCourier, registerCourierValidation, login, loginValidation, refresh, refreshValidation, getProfile, updatePushToken };
